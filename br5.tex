...
\end{proof}
Similarly, one can generalize Lemma~\ref{lemexplore} to apply to adding characters to the right of $L_i$ rather than
to the left.
...
%A sample tree of decompositions is displayed in Figure~\ref{decomptree}.

\begin{sidewaysfigure}\begin{tiny}\[ \begin{tikzpicture}
\node{$\1\{\0,\1,\6\}^*\1$}[sibling distance=6.75cm]
	child{node{$\1\0\{\0,\1,\6\}^*\1$}[sibling distance=2.5cm]
	...
	child{node{$\1\1\{\0,\1,\6\}^*\1$}[sibling distance=4.5cm]
		child{node{$\1\1\{\0,\1,\6\}^*\1$}[sibling distance=1.5cm]
...
\newblock {\it PRIMO - Primality Proving},
