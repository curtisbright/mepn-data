...
\end{corollary}
\begin{proof}
By Lemma~\ref{lemone}, $N:=\gcd([xz]_b,[xy_1z]_b,\dotsc,[xy_nz]_b)$ divides all numbers of the form $[x\{y_1,\dotsc,y_n\}^*z]_b$.
By the size condition $N>1$ is strictly less than each such number, and so is a nontrivial divisor.
\end{proof}
...
We also generalize this to the following corollary.
\begin{corollary}
...
\end{corollary}
\begin{proof}
By Lemma~\ref{lemone}, $N_0$ divides all numbers of the form $[x\{y_1,\dotsc,y_n\}^*z]_b$ of even length and $N_1$ divides all such numbers
of odd length.  By the size conditions, $N_0$ and $N_1$ are nontrivial divisors.
\end{proof}
\begin{example}
Since $\gcd([\6]_9,[\6\1\1]_9)=2$, every number of the form $\6\1^*$ of even length is divisible by $2$ in base $9$.
Since $\gcd([\6\1]_9,[\6\1\1\1]_9)=5$, every number of the form $\6\1^*$ is divisible by $5$ in base $9$.  Since these numbers lie strictly between
...
$N_i$ divides all numbers of the form $[x\{y_1,\dotsc,y_n\}^*z]_b$ of length congruent to $i$ mod $k$.
