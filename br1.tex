...
$[L]_b = \lbrace [x]_b \ : \ x \in L \rbrace$.
We use the convention that ${\tt A} = 10$, ${\tt B} = 11$, and so forth,
...
$L^n = \overbrace{LL\cdots L}^n$ and $L^* = \bigcup_{i \geq 0} L^i$.
...
and $\Sigma^* - \subb(L) = \supe(\Sigma^* - \supe(L))$.
...
\noindent (2) while ($L \not= \emptyset$) do \\
\hphantom{X} \quad\quad (3) choose $x$, a shortest string in $L$ \\
\hphantom{X} \quad \quad (4) $ M := M \ \cup \ \lbrace  x \rbrace$ \\
\hphantom{X} \quad \quad (5) $ L := L - \supe(\lbrace x \rbrace) $ \\
...
If \[N_0:=\gcd \left( \{ [xz]_b \} \ \cup \ [xL^2 z]_b \right) \]
and
\[N_1:= \gcd  \left( \{ [xLz]_b \} \ \cup \ [xL^3z]_b \right) \]
...
\begin{example}
Take $L_1 = \{ \0 \}$ and $(x_1,x_2, x_3) = (\9, \8, \1)$.
...
A sketch of the algorithm is needed.

It would also be good to give the tree of decompositions for some
small base, say $b = 9$.
...
Our results assume that a number which has passed several Miller--Rabin
...
Write $S_b = \lbrace (n)_b \ : \ n \geq 4 \text{ is composite} \rbrace$.
...
b & \lvert M(L_b)\rvert & \max\limits_{x\in M(L_b)}\lvert x\rvert \\ \hline
2 & 3 & 4 \\
3 & 4 & 3 \\
4 & 9 & 3 \\
5 & 10 & 3 \\
