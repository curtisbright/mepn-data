...
a prime being found, except for those in base~29, which have been tested up to length $50000$.
...
Base 28: $\mathtt{OA^*F}$ \\
Base 29: $\mathtt{1A^*}$, $\mathtt{68L0^*6}$, $\mathtt{AMP^*}$, $\mathtt{C^*FK}$, $\mathtt{E9C^*}$, $\mathtt{F^*OPF}$, $\mathtt{FKI^*}$, $\mathtt{F^*IK}$, $\mathtt{F^*OP}$, $\mathtt{I^*FK}$, $\mathtt{LP09^*}$, $\mathtt{LPC^*9}$, $\mathtt{L^*CE}$, $\mathtt{LOG^*}$, $\mathtt{OOPS^*A}$, $\mathtt{O0^*FPL}$, $\mathtt{PC^*}$, $\mathtt{PCL^*}$, $\mathtt{POL^*0O}$, $\mathtt{PPPL^*O}$, $\mathtt{P0^*OPR}$, $\mathtt{Q^*GL}$, $\mathtt{Q^*LO}$, $\mathtt{RM^*G}$
\end{figure}
...
29\mathrlap{^*} & \geq57273 & \geq54180 & 24 \\
30 & 220 & 1024 & 0
\end{array}\]
