...
For bases between 17 and~30 a lower bound on the size and width of
...
The following unsolved families have been tested to at least length $125000$ without
a prime being found, except for those in base~29, which have been tested up to length $45000$.
...
Base 23: $\mathtt{9E^*}$ \\
Base 25: $\mathtt{6MF^*9}$, $\mathtt{96^*M}$, $\mathtt{CM1^*}$, $\mathtt{EE1^*}$, $\mathtt{E1^*E}$, $\mathtt{EFO^*}$, $\mathtt{F1^*F1}$, $\mathtt{F0^*KO}$, $\mathtt{F0K^*O}$, $\mathtt{LOL^*8}$, $\mathtt{M1^*F1}$, $\mathtt{M10^*8}$, $\mathtt{OL^*8}$ \\
Base 26: $\mathtt{A^*6F}$, $\mathtt{I^*GL}$ \\
Base 27: $\mathtt{80^*9A}$, $\mathtt{999G^*}$, $\mathtt{CL^*E}$, $\mathtt{EI^*F8}$, $\mathtt{F^*9FM}$ \\
Base 28: $\mathtt{OA^*F}$ \\
Base 29: $\mathtt{1A^*}$, $\mathtt{68L0^*6}$, $\mathtt{AMP^*}$, $\mathtt{C^*FK}$, $\mathtt{E9C^*}$, $\mathtt{F^*OPF}$, $\mathtt{FKI^*}$, $\mathtt{F^*IK}$, $\mathtt{F^*OP}$, $\mathtt{I^*FK}$, $\mathtt{LP09^*}$, $\mathtt{LPC^*9}$, $\mathtt{L^*CE}$, $\mathtt{LOG^*}$, $\mathtt{OOPS^*A}$, $\mathtt{O0^*FPL}$, $\mathtt{PC^*}$, $\mathtt{PL^*C}$, $\mathtt{PCL^*}$, $\mathtt{POL^*0O}$, $\mathtt{PPPL^*O}$, $\mathtt{P0^*OPR}$, $\mathtt{Q9R0^*9}$, $\mathtt{Q^*GL}$, $\mathtt{Q^*LO}$, $\mathtt{RM^*G}$
\end{figure}
