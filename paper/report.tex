...
The following unsolved families have been tested to at least length $130000$ without
a prime being found, except for those in base~29, which have been tested up to length $60000$.
...
Base 28: $\mathtt{OA^*F}$ \\
Base 29: $\mathtt{1A^*}$, $\mathtt{68L0^*6}$, $\mathtt{AMP^*}$, $\mathtt{C^*FK}$, $\mathtt{F^*OPF}$, $\mathtt{FKI^*}$, $\mathtt{F^*OP}$, $\mathtt{I^*FK}$, $\mathtt{LP09^*}$, $\mathtt{LPC^*9}$, $\mathtt{L^*CE}$, $\mathtt{LOG^*}$, $\mathtt{OOPS^*A}$, $\mathtt{O0^*FPL}$, $\mathtt{PC^*}$, $\mathtt{PCL^*}$, $\mathtt{POL^*0O}$, $\mathtt{PPPL^*O}$, $\mathtt{Q^*GL}$, $\mathtt{Q^*LO}$, $\mathtt{RM^*G}$
\end{figure}
...
29\mathrlap{^*} & \geq57276 & \geq61503 & 21 \\
30 & 220 & 1024 & 0
\end{array}\]
